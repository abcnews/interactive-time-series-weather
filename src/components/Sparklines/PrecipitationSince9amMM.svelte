<script lang="ts">
  ...
  const gradientColors = metricProperties.rain.gradientColours;
...
<SparklineViz
  ...
  loadData={async () => {
    const charts = await fetchData(
      LOCATIONS_URL,
      'https://abcnewsdata.sgp1.digitaloceanspaces.com/data-time-series-weather/precipitationSince9amMM.json',
      locations
    );
    return { charts };
  }}
